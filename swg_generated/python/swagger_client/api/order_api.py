...
class OrderApi(object):
    ...
    def order_info_all(self, **kwargs):  # noqa: E501
        ...
        :param str kind: 주문 유형 - normal : 일반 주문 - watch : 예약 주문 
    ...
    def order_info_all_with_http_info(self, **kwargs):  # noqa: E501
        ...
        all_params = ['market', 'state', 'states', 'uuids', 'identifiers', 'kind', 'page', 'limit', 'order_by']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
    ...
    def order_new(self, market, side, volume, price, ord_type, **kwargs):  # noqa: E501
        """주문하기  # noqa: E501
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.order_new(market, side, volume, price, ord_type, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param str market: 마켓 ID (필수)  (required)
        :param str side: 주문 종류 (필수) - bid : 매수 - ask : 매도  (required)
        :param str volume: 주문량 (지정가, 시장가 매도 시 필수)  (required)
        :param str price: 주문 가격. (지정가, 시장가 매수 시 필수)  ex) KRW-BTC 마켓에서 1BTC당 1,000 KRW로 거래할 경우, 값은 1000 이 된다. ex) KRW-BTC 마켓에서 1BTC당 매도 1호가가 500 KRW 인 경우, 시장가 매수 시 값을 1000으로 세팅하면 2BTC가 매수된다. (수수료가 존재하거나 매도 1호가의 수량에 따라 상이할 수 있음)  (required)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.order_new_with_http_info(market, side, volume, price, ord_type, **kwargs)  # noqa: E501
        else:
            (data) = self.order_new_with_http_info(market, side, volume, price, ord_type, **kwargs)  # noqa: E501
            return data

    def order_new_with_http_info(self, market, side, volume, price, ord_type, **kwargs):  # noqa: E501
        """주문하기  # noqa: E501

        ## 주문 요청을 한다.  **NOTE**: 원화 마켓 가격 단위를 확인하세요.  원화 마켓에서 주문을 요청 할 경우, [원화 마켓 주문 가격 단위](https://docs.upbit.com/docs/market-info-trade-price-detail)를 확인하여 값을 입력해주세요.  **NOTE**: identifier 파라미터 사용  `identifier`는 서비스에서 발급하는 `uuid`가 아닌 이용자가 직접 발급하는 키값으로, 주문을 조회하기 위해 할당하는 값입니다. 해당 값은 사용자의 전체 주문 내 유일한 값을 전달해야하며, 비록 주문 요청시 오류가 발생하더라도 같은 값으로 다시 요청을 보낼 수 없습니다.  주문의 성공 / 실패 여부와 관계없이 중복해서 들어온 `identifier` 값에서는 중복 오류가 발생하니, 매 요청시 새로운 값을 생성해주세요.  **NOTE**: 시장가 주문  시장가 주문은 `ord_type` 필드를 `price` or `market` 으로 설정해야됩니다. 매수 주문의 경우 `ord_type`을 `price`로 설정하고 `volume`을 `null` 혹은 제외해야됩니다. 매도 주문의 경우 `ord_type`을 `market`로 설정하고 `price`을 `null` 혹은 제외해야됩니다.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.order_new_with_http_info(market, side, volume, price, ord_type, async_req=True)
        >>> result = thread.get()
        ...
        all_params = ['market', 'side', 'volume', 'price', 'ord_type', 'identifier']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        if ('volume' not in params or
            ...
            raise ValueError("Missing the required parameter `volume` when calling `order_new`")  # noqa: E501
